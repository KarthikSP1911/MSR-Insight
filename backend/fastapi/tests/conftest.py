...
def mock_google_genai():
    with patch('app.services.rag.service.ChatGoogleGenerativeAI') as MockLLM:
...
@pytest.fixture(autouse=True)
def mock_embeddings():
    with patch('app.services.rag.service.GoogleGenerativeAIEmbeddings') as MockEmbeddings:
