...
@patch('app.services.rag.service.fetch_and_sync_documents')
def test_sync_data(mock_fetch_sync):
    mock_fetch_sync.return_value = {"status": "success", "students": 1, "chunks": 5}
    
    service = RAGService()
    
    with patch('app.services.rag.service.RAGService.vector_store', new_callable=PropertyMock) as mock_vs_prop:
        mock_vs = MagicMock()
...
@patch('app.services.rag.service.get_ensemble_retriever')
