...
// Update Components
import { ProgressToast } from "@/components/dashboard/ProgressToast";
...
export default function StudentDashboard() {
    ...
    const detailsBlob = useMemo(() => detailedData?.details || detailedData || {}, [detailedData]);
    ...
    const currentCgpa = useMemo(() => {
        const val = (detailsBlob.cgpa ?? detailedData?.cgpa ?? "").toString().trim();
        return val || null;
    }, [detailsBlob, detailedData]);
    ...
    const stdUsn = useMemo(() => detailedData?.usn || detailsBlob.usn || "", [detailedData, detailsBlob]);
        ...
        const fetchProfile = async () => {
            ...
            try {
                ...
                if (detailedResp.data.success && detailedResp.data.data) {
                    const data = detailedResp.data.data;
                ...
                if (err.response?.status === 401) { localStorage.clear(); router.push("/student-login"); }
    ...
    const handleUpdate = async () => {
        if (isCooldownActive || updateStatus === 'loading') return;
        ...
        const progressInterval = setInterval(() => {
            setUpdateProgress(prev => {
                ...
                return Math.min(prev + (prev < 50 ? 2 : prev < 80 ? 1 : 0.5), 95);
            });
        }, 300);
        ...
        try {
            const sessionId = localStorage.getItem("studentSessionId");
            const usn = localStorage.getItem("studentUsn");
            const response = await axios.post(`${API_BASE_URL}/api/report/update`, { usn }, {
                headers: { "x-session-id": sessionId },
            });
            ...
            if (response.data.success) {
                ...
                setNextAllowedAt(new Date(Date.now() + 30 * 60 * 1000).toISOString());
                setTimeout(() => { setUpdateStatus(null); setUpdateProgress(0); }, 3000);
            } else if (response.data.allowed === false) {
                setNextAllowedAt(response.data.nextAllowedAt);
    ...
    if (loading) return (
        <div className="dashboard-loading">
            ...
            <p className="loading-text">Synchronizing your dashboard...</p>
        </div>
    );

    return (
        <div className="student-dashboard-container">
            <ProgressToast status={updateStatus} progress={updateProgress} onClose={() => setUpdateStatus(null)} />
